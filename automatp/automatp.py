...
if rap_prom is not None or ipra is not None:
    ...
    try:
        ...
        #siur
        IPRA_EO  = pd.read_excel(ipra,sheet_name='IPRA EO')
        ###
        ...
        IPRA_WHA[['Indeks','Ilość min.','ID promocji']]=IPRA_WHA[['Indeks','Ilość min.','ID promocji']].fillna('0')
        IPRA_WHA = IPRA_WHA.astype({'Indeks':int,'ID promocji':int,'Ilość min.':int})
        ...
        nowy1['Rabat IPRA BWH']=nowy1['Rabat IPRA BWH']*100
        nowy1['Rabat Promocyjny']=nowy1['Rabat Promocyjny']*100
        ##
